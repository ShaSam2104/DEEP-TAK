...
class Game:
	...
	def __init__(self, n):
		...
		self.board = [[] for i in range(self.total_squares)]
		self.turn = 0
		if n == 5:
			self.max_flats = 21
		...
		self.players.append(Game.Player(self.max_flats, self.max_capstones))
		self.all_squares = [self.square_to_string(i) for i in range(self.total_squares)]
	...
	def generate_stack_moves(self, square):
		...
		for num in range(min(size, self.n)):
